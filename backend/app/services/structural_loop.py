...
# Per-(project_id, layout_key) async lock to guard against concurrent design runs.
# Cloud Run typically runs few instances; cross-instance double-runs are rare and
# cost only a wasted structapi call. This in-process guard prevents same-instance
# concurrent runs (e.g., agent-chat tool + Structural tab both triggering simultaneously).
_inflight_lock = asyncio.Lock()
_inflight_runs: set[tuple[str, str]] = set()
...
class DesignRunInProgress(Exception):
    """Raised when a design run is already in progress for the same layout."""
...
async def _check_and_register_inflight(project_id: str, layout_key: str) -> None:
    """Check if a design run is in progress for this (project_id, layout_key).

    If already in progress, raises DesignRunInProgress.
    If not, registers it as in-progress.
    Caller must call _unregister_inflight() in a finally block to release.
    """
    async with _inflight_lock:
        key = (project_id, layout_key)
        if key in _inflight_runs:
            raise DesignRunInProgress(
                f"Design run already in progress for layout {layout_key}"
            )
        _inflight_runs.add(key)


def _unregister_inflight(project_id: str, layout_key: str) -> None:
    """Remove the design run from the in-flight set. Called in finally block."""
    key = (project_id, layout_key)
    _inflight_runs.discard(key)
...
async def run_design_loop(
    ...
    *,
    ...
    for the route to translate. Raises DesignRunInProgress if a concurrent
    run is already underway for this layout."""
    # Guard: reject if a design run is already in progress for this layout
    await _check_and_register_inflight(project.id, revision.layout_key)
    ...
    try:
        ...
        for iteration in range(1, MAX_ITERATIONS + 1):
            ...
            result = await structagent_client.design_building(
                payload,
                correlation_id=f"{project.id}:{revision.layout_key}:i{iteration}",
            )
            ...
            approved_layout = layout_store.engine_layout_from_geometry(
                approved_geometry
            )
        ...
        _unregister_inflight(project.id, revision.layout_key)
