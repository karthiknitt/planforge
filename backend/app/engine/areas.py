...
def validate_voids(below: FloorPlan, above: FloorPlan) -> None:
    ...
    No production code path emits `is_void=True` rooms yet — floor-N-over-
    floor-N-1 voids (capability 18 in `solver_capability_gaps.md`) are not
    generated by the solver, only hand-authorable via the editor. This is a
    no-op today by construction (the `if not v.is_void: continue` above skips
    every real room) and becomes load-bearing once a producer exists; call it
    from `generator.py`'s multi-floor assembly at that point.
    """
