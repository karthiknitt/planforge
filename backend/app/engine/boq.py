...
class BOQResult:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "generic_total_cost": round(self.generic_total_cost)
...
class QuantityEngine:
    ...
    def calculate(
        ...
        result.line_items.append(
            BOQLineItem("1", "Built-up area (Ground floor)", gf_area, "sqm")
        )
        result.line_items.append(
        ...
        result.line_items.append(
            BOQLineItem(
                "6",
                "Brick masonry — external walls (230 mm)",
                round(ext_wall_vol, 3),
                "cum",
        ...
        result.line_items.append(
            BOQLineItem(
                "7",
                "Brick masonry — internal walls (115 mm)",
                round(int_wall_vol, 3),
                "cum",
                rate=bricks_per_1000_rate,
        ...
        # ── Plaster ─────────────────────────────────────────────────────────
        plaster_area = (
            ext_wall_len * FLOOR_HEIGHT_M * 2 * 2
            + self._internal_wall_area(gf, ff, iwt) * 2
        )  # both sides
        result.line_items.append(
            BOQLineItem(
                "8",
                "Cement plaster (12 mm thick, internal + external)",
                round(plaster_area, 1),
                "sqm",
            )
        )
        ...
        result.line_items.append(
            BOQLineItem(
                "9",
                "Flooring (vitrified tiles, avg 600×600 mm)",
                round(total_area, 1),
                "sqm",
        ...
        result.line_items.append(
            BOQLineItem(
                "10",
                f"Excavation for column footings ({col_count} nos)",
                round(footing_vol, 1),
                "cum",
            )
        )
        ...
        result.line_items.append(
            BOQLineItem("11", "PCC (1:4:8) under footings", round(pcc_vol, 3), "cum")
        )
        ...
        result.line_items.append(
            BOQLineItem(
                "12",
                "Labour — civil works (masonry, concrete, plaster)",
                round(total_area_sqft, 1),
                "sqft",
        ...
        result.line_items.append(
            BOQLineItem(
                "13",
                "Labour — finishing works (tiles, paint, carpentry)",
                round(total_area_sqft, 1),
                "sqft",
        ...
        result.line_items.append(
            BOQLineItem(
                "14",
                f"Plumbing and sanitary ({bathroom_count} bathrooms)",
                bathroom_count,
                "nos",
                rate=plumbing_rate,
        ...
        result.line_items.append(
            BOQLineItem(
                "15",
                "Electrical works (wiring, DB, fixtures)",
                round(total_area_sqft, 1),
                "sqft",
        ...
        result.line_items.append(
            BOQLineItem(
                "16",
                "River sand (masonry + plaster + concrete)",
                round(sand_cft, 1),
                "cft",
                rate=sand_rate,
        ...
        result.line_items.append(
            BOQLineItem(
                "17",
                "Coarse aggregate 20mm (concrete work)",
                round(agg_cft, 1),
                "cft",
                rate=agg_rate,
    ...
    def _internal_wall_volume(self, gf: FloorPlan, ff: FloorPlan, iwt: float) -> float:
        ...
        for plan in [gf, ff]:
            ...
            all_y = ys[1:-1]
            for x in all_x:
                span_d = max(r.y + r.depth for r in plan.rooms) - min(
                    r.y for r in plan.rooms
                )
                total += span_d * FLOOR_HEIGHT_M * iwt
            for y in all_y:
                span_w = max(r.x + r.width for r in plan.rooms) - min(
                    r.x for r in plan.rooms
                )
