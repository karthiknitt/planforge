...
def _make_layout(
    gf_rooms, ff_rooms=None, sf_rooms=None, basement_rooms=None, gf_type="ground"
) -> Layout:
    gf = FloorPlan(floor=0, floor_type=gf_type, rooms=gf_rooms or [])
    ff = FloorPlan(floor=1, floor_type="first", rooms=ff_rooms or [])
    sf = (
        FloorPlan(floor=2, floor_type="second", rooms=sf_rooms)
    ...
    bsmt = (
        FloorPlan(
            ...
            needs_mech_ventilation=True,
        )
        if basement_rooms is not None
        else None
    )
    return Layout(
        id="T",
