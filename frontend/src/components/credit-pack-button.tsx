...
export function CreditPackButton({ packId, label, highlight }: CreditPackButtonProps) {
  ...
  async function handleCheckout() {
    ...
    try {
      ...
      if (!orderRes.ok) {
        ...
        throw new Error((data as { detail?: string })?.detail ?? "Failed to create payment order");
      }

      const { order_id, amount, key_id, credits } = (await orderRes.json()) as {
        order_id: string;
        amount: number;
        key_id: string;
        credits: number;
      ...
      const rzp = new (
        ...
        currency: "INR",
        name: "PlanForge",
        description: `${credits} Project Credit${credits > 1 ? "s" : ""}`,
        prefill: {
        ...
        handler: async (response: { razorpay_payment_id: string; razorpay_signature: string }) => {
          ...
          router.push("/dashboard?credits_added=1");
        },
        modal: {
