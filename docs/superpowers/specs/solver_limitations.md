...
| L (repair pass forces a door onto gapped parking) | Fixed (2026-08-12) | `_repair_connectivity()`'s `unreachable` filter excluded only `"passage"` — a genuinely gapped parking room (checklist item 3: real physical clearance, not just RoomType, so `_NO_DOOR_TYPES` in the main per-room loop skips it) always came back "unreachable" from the reachability BFS and got a forced pedestrian door on its own exterior wall anyway, undoing `_NO_DOOR_TYPES`'s exclusion. Now excludes `_PARKING_TYPES` too — a car porch is reachable only from the driveway/outside, and being unreachable via an interior door is the correct, intended topology, not a defect. New regression test `test_gapped_parking_gets_no_repair_door` in `backend/tests/test_plan_openings.py`. Found via user-reported visual review of the Assamese-07 re-render: a spurious door + window had appeared on the car porch's exterior wall where the source plan shows it fully open to the driveway. |
| A, B, D, E | Out of scope | accepted schema limits / methodology notes — see plan |

### New limitations found (2026-08-12, user visual review of Assamese-07 re-render)

- **K. No "open-sided room" concept — a car porch/carport always gets full walls on every side.** Even after fix L above removes the spurious *door*, `derive_walls()` still draws a real, solid `ewt` wall around a parking room's entire perimeter, since every `Room` is schema-required to be a fully enclosed rectangle (limitation A). A real Indian carport is typically open on 1–2 sides under a slab/roof on columns, not walled — the source Assamese-07 sheet shows no wall line at all on the car porch's road/driveway-facing sides. Fixing this properly needs a real schema addition (e.g. an `open_sides` field on `Room`, or a dedicated carport-drawing path in `derive_walls()`/`pdf.py` that draws columns instead of a wall on the driveway-facing edge) — not a small patch. Documented, not fixed.
- **M. No compound/boundary-wall or site-boundary (green/landscaped-area) rendering at all.** `render_pdf()`/`pdf.py` draws the setback *dimension* lines (`_draw_setback_callouts`) and a dashed setback rectangle, but there is no compound-wall entity and no landscaped-area fill — reference marketing sheets typically show a solid boundary/compound wall at the plot edge (often with a gate) plus a grass texture filling the setback margin, which reads as more dimensionally "grounded" than PlanForge's dashed-line-only setback representation. This is a real feature gap, not a bug in existing logic — no code currently attempts this and nothing is broken; it would be new site-plan drawing work. Documented, not fixed.
